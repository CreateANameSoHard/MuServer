...
public:
    ...
    LFUCache(size_t capacity, int maxAverageNum = 10)
        : capacity_(capacity), minFreq_(INT8_MAX), maxAvgFreq_(maxAverageNum),
          curAvgFreq_(0), curTotalFreq_(0)
    {
    }
...
private:
    ...
    void increaseFreq();              // 增加平均访问等频率
    void decreaseFreq(int num);  // 减少平均访问等频率
...
private:
    ...
    int maxAvgFreq_;                                              // 最大平均访问频次
    int curAvgFreq_;                                              // 当前平均访问频次
    int curTotalFreq_;                                                // 当前访问所有缓存次数总数
...
template <typename Key, typename Value>
void LFUCache<Key, Value>::increaseFreq()
{
    curTotalFreq_++;
    if (nodeMap_.empty())
        curAvgFreq_ = 0;
...
template <typename Key, typename Value>
void LFUCache<Key, Value>::decreaseFreq(int num)
{
    // 减少平均访问频次和总访问频次
    curTotalFreq_ -= num;
    if (nodeMap_.empty())
...
void LFUCache<Key, Value>::handleOverMaxAverageNum()
{
    ...
    // 当前平均访问频次已经超过了最大平均访问频次，所有结点的访问频次- (maxAvgFreq_ / 2)
